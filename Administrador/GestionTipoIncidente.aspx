...
<uc:Header Titulo="Fedisal - Tipos de Incidentes" runat="server" ID="Header" />
    ...
    <body>
        <header>
            <uc:Menu Titulo="Gestión de Tipos de Incidente" runat="server" ID="Menu" />
        </header>
                ...
                <form id="form1" runat="server">
                    <asp:GridView ID="gdIncidentes" runat="server" CssClass="centered" AllowPaging="True" 
                        ...
                        <Columns>
                            ...
                            <asp:BoundField DataField="InicidentesRegistrados" HeaderText="Inicidentes Relacionados" SortExpression="InicidentesRegistrados" />
                            <asp:BoundField DataField="descripcion" HeaderText="Descripcion" SortExpression="descripcion" />
                            <asp:TemplateField HeaderText="Modificar" SortExpression="idTipoIncidente">
                                <ItemTemplate>
                                    <asp:HyperLink NavigateUrl='<%# string.Concat("/Administrador/RegistroTipoIncidente.aspx?id=", Eval("idTipoIncidente")) %>' ID="btnModificarGV" runat="server" Visible="true" CssClass="btnModificar waves-effect waves-light btn modal-trigger" Text='Modificar' />
                                </ItemTemplate>
                           </asp:TemplateField>
                            <asp:TemplateField HeaderText="Eliminar" SortExpression="idCarrera">
                                <ItemTemplate>
                                    <asp:HyperLink NavigateUrl="#mdlEliminar" ID="btnEliminarGV" runat="server" idTipoInicidente='<%# Eval("idTipoIncidente") %>' Visible='<%# (Convert.ToInt32(Eval("InicidentesRegistrados")) > 0) ? false : true %>' CssClass="eliminarModal waves-effect waves-light btn modal-trigger" Text='Eliminar' />
                                </ItemTemplate>
                            </asp:TemplateField>
                        </Columns>
                    </asp:GridView>
